...
from shape.game_logic import get_top_moves
...
class BoardView(QWidget):
    ...
    def draw_coordinates(self, painter):
        ...
        for i in range(self.board_size):
            ...
            painter.drawText(QRectF(bottom_box.x(), bottom_box.y(), self.cell_size, self.cell_size*0.5), Qt.AlignHCenter | Qt.AlignTop, letter)

            left_box = self.intersection_coords(i+0.5,-1)
            painter.drawText(QRectF(left_box.x(), left_box.y(), self.cell_size * 0.5, self.cell_size), Qt.AlignVCenter | Qt.AlignRight, str(i+1))
    ...
    def draw_heatmap(self, painter, policy, show_text, sampling_settings):
        ...
        if self.main_window.game_logic.current_node.move and self.main_window.game_logic.current_node.move[1] == "pass" and self.main_window.game_logic.current_node.parent.parent is None:
        ...
        if heatmap_mean_prob is not None:
            top_moves, _ = get_top_moves(heatmap_mean_prob, self.board_size, secondary_data=(heatmap_mean_rank,),  **sampling_settings)
    ...
    def draw_heatmap_point(self, painter, row, col, total_prob, max_prob, mean_rank, color, show_text):
        ...
        if show_text:
            ...
            painter.drawText(QRectF(x - self.cell_size/2, y - self.cell_size/2, self.cell_size, self.cell_size), Qt.AlignCenter, text)